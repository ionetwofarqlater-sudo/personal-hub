...
export default function SavedComposer({ onAdd, replyTo, onCancelReply }: Props) {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      // Attach filename + size into metadata via a custom key we handle in useSavedItems
      // We encode it temporarily in content so the hook can strip it out,
      // but a cleaner approach is to let onAdd accept a metadata override.
      // Since our CreateSavedItemInput doesn't have metadata, we store in title.
      // Instead, pass extra fields via a typed extension:
      (input as CreateSavedItemInput & { _meta?: Record<string, string> })._meta = {
        filename: file.name,
      ...
      if (
        ...
        setUploadError(
          'Bucket "saved-media" не знайдено або він приватний. ' +
          'Створіть публічний bucket у Supabase Storage → New bucket → "saved-media" → Public: on.'
        );
      } else {
        setUploadError('Помилка завантаження файлу. Спробуй ще раз.');
      }
      // Reset back to text mode so the composer stays usable
      setType('text');
    } finally {
      setUploading(false);
    }
      ...
      {isUploadMode ? (
        ...
        >
          {uploading ? (
            ...
            <><FileUp className="w-4 h-4" /> Натисни, щоб вибрати {type === 'image' ? 'зображення' : 'файл'}</>
          )}
        </button>
      ) : (
